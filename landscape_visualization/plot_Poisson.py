...
from _aux.PINN_data_aux import LossData, get_PINN
...
def get_errors(model, error_type, loss_dict):
    if error_type == "u":
        error = torch.sqrt(torch.mean((u_exact_test - model(grid_test).reshape(-1)) ** 2))
    if error_type == "loss_oper":
        error = loss_dict["loss_oper"]
    if error_type == "loss_bnd":
        error = loss_dict["loss_bnd"]
    if error_type == "loss_total":
        error = loss_dict["loss"]
    return error
...
if __name__ == '__main__':
    ...
    grid_res = 100 #TODO add to arguments
    ...
    for i in range(trajectory_models.shape[0]):
        ...
        loss_compute = LossData(equation_model.solution_cls)
        loss_dict = loss_compute.evaluate()
        loss = get_errors(model_repopulated, loss_type, loss_dict).detach()
        ####
