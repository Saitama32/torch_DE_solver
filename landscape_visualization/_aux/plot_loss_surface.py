...
class PlotLossSurface:
    ...
    def get_trajectories_and_load_model(self):
        ...
        trajectory_data_loader, transform = get_trajectory_dataloader(pt_files, self.batch_size, self.path_to_plot_model )
        trajectory_dataset = trajectory_data_loader.dataset
        input_dim = trajectory_dataset[0].shape[0]

        ##########MODEL
        # load the model
        best_model = UniformAutoencoder(input_dim, self.num_of_layers, self.latent_dim, h=self.layers_AE).to(self.device)  
        best_model.load_state_dict(torch.load(self.path_to_plot_model))
    ...
    def plotting(self, trajectory_losses: torch.Tensor, original_trajectory_losses: torch.Tensor, 
        ...
        df.to_csv(os.path.join(self.path_to_plot_model_directory, 'summary_'+self.loss_name + '_' + self.loss_type+'.csv'), quoting=csv.QUOTE_NONNUMERIC, index=False)
        ds = torch.stack(ds)
        ...
        CS = plt.contour(grid_xx.detach().cpu().numpy(), grid_yy.detach().cpu().numpy(), density,  levels=levels_density, vmin=density_vmin, vmax=density_vmax)
        ...
        fig.savefig(os.path.join(self.path_to_plot_model_directory, 'map_'+self.loss_type+'_grid_density.pdf'), dpi=300, bbox_inches='tight', format='pdf')
        fig.show()
        ...
        for plot_ in plots_:
            ...
            CS = plt.contour(grid_xx.detach().cpu().numpy(), grid_yy.detach().cpu().numpy(), grid_losses.detach().cpu().numpy(), levels=levels, norm =norm)
            ...
            fig.savefig(os.path.join(self.path_to_plot_model_directory, 'map_'+self.loss_type+'_'+self.loss_name+'_'+plot_+'.pdf'), dpi=300, bbox_inches='tight', format='pdf')

            fig.show()

    def plotting_equation_loss_surface(self, u_exact_test: torch.Tensor, grid_test: torch.Tensor, grid: torch.Tensor, domain: Domain, equation: Equation, boundaries: Conditions, PINN_layers: list):
