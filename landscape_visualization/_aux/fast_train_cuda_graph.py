...
def _warmup_alloc(model, optim, static_x, rec_weight):
...
def make_train_cudagraph(model, optim, batch_shape, rec_weight, device="cuda", dtype=torch.float32):
    model.train()
    ...
    _warmup_alloc(model, optim, static_x, rec_weight)
    ...
    def step(batch_gpu: torch.Tensor, lr: float):
