...
experiment = start(
  ...
  project_name="rlpinn_heat_3d_tolerance",
  workspace="saitama32"
)
...
experiment.log_parameters({
    "param": "v_1",
    "reward_function": "v_2",
    "description": "farm_transitions_Heat_3d_basic_RL_optimizer"
})
...
def exact_func(grid):
    x, y, z, t = grid[:, 0], grid[:, 1], grid[:, 2], grid[:, 3]
    sln = torch.sum((torch.sin(k * x[:, None]) +
                     torch.sin(k * y[:, None]) +
                     torch.sin(k * z[:, None])) *
                    torch.exp(-eps * k ** 3 * t[:, None]))
    return sln


def heat_3d_analytic_experiment(grid_res):
    exp_dict_list = []

    x_min, x_max = 0, 2 * torch.pi
    y_min, y_max = 0, 2 * torch.pi
    z_min, z_max = 0, 2 * torch.pi
    t_max = 0.01
    ...
    domain_test.variable('z', [z_min, z_max], grid_test_res)
    ...
    init_func = lambda grid: torch.sum(torch.sin(k * grid[:, 0][:, None]) +
                                       torch.sin(k * grid[:, 1][:, None]) +
                                       torch.sin(k * grid[:, 2][:, None]))

    # u(x, y, 0)
    boundaries.dirichlet({'x': [x_min, x_max], 'y': [y_min, y_max], 'z': [z_min, z_max], 't': 0}, value=init_func)

    # Boundary conditions (periodic): ##################################################################################

    # u(0, y, z, t) = u(2*pi, y, z, t)
    boundaries.periodic([{'x': x_min, 'y': [y_min, y_max], 'z': [z_min, z_max], 't': [0, t_max]},
                         {'x': x_max, 'y': [y_min, y_max], 'z': [z_min, z_max], 't': [0, t_max]}])

    # u(x, 0, z, t) = u(x, 2*pi, z, t)
    boundaries.periodic([{'x': [x_min, x_max], 'y': y_min, 'z': [z_min, z_max], 't': [0, t_max]},
                         {'x': [x_min, x_max], 'y': y_max, 'z': [z_min, z_max], 't': [0, t_max]}])

    # u(x, y, 0, t) = u(x, y, 2*pi, t)
    boundaries.periodic([{'x': [x_min, x_max], 'y': [y_min, y_max], 'z': z_min, 't': [0, t_max]},
                         {'x': [x_min, x_max], 'y': [y_min, y_max], 'z': z_max, 't': [0, t_max]}])

    equation = Equation()

    # Operator: du/dt -  epsilon * (u_xx + u_yy + u_zz) = 0
    ...
    img_dir = os.path.join(os.path.dirname(__file__), 'heat_3d_analytic_img')
