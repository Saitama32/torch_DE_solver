...
def burgers_1d_periodic_experiment(grid_res):
    ...
    boundaries.dirichlet({'x': [x_min, x_max], 't': 0},
                         value=lambda grid: torch.sum(torch.sin(k * grid[:, 0][:, None]), dim=1, keepdim=True))
    ...
    img_dir = os.path.join(os.path.dirname(__file__), 'burgers_1d_periodic_img')
    ...
    model.train(optimizer, 1e5, save_model=True, callbacks=[cb_es, cb_plots, cb_cache])
...
for grid_res in range(100, 1001, 100):
    for _ in range(nruns):
        exp_dict_list.append(burgers_1d_periodic_experiment(grid_res))

import pandas as pd

exp_dict_list_flatten = [item for sublist in exp_dict_list for item in sublist]
df = pd.DataFrame(exp_dict_list_flatten)
df.to_csv('examples/benchmarking_data/burgers_1d_periodic_experiment_50_500_cache={}.csv'.format(str(True)))
