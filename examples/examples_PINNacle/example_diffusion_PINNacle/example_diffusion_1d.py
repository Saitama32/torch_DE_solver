...
def exact_func(grid):
    x, t = grid[:, 0], grid[:, 1]
    sln = torch.sum(torch.sin(k * x[:, None]) * torch.exp(-epsilon * k ** 2 * t[:, None]), dim=1, keepdim=True)
    return sln
...
df_u.to_csv('examples/benchmarking_data/diffusion_1d_experiment_100_1000_cache_u_func={}.csv'.format(str(True)))
