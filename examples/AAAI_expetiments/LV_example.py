...
def experiment_data_amount_LV_fourier(grid_res,exp_name='LV_fourier',save_plot=True):
    solver_device('cuda')
    exp_dict_list = []
...
if __name__ == '__main__':
    ...
    for grid_res in range(10, 101, 10):
        for _ in range(nruns):
            ...
            df.to_csv('examples\\AAAI_expetiments\\results\\LV_PSO_{}.csv'.format(grid_res))
    ...
    #        exp_dict_list_flatten = [item for sublist in exp_dict_list for item in sublist]
    #        df = pd.DataFrame(exp_dict_list_flatten)
    #        df.to_csv('examples\\AAAI_expetiments\\results\\LV_lam_{}.csv'.format(grid_res))
    ...
    #for grid_res in range(10, 101, 10):
    #    for _ in range(nruns):
    #        exp_dict_list.append(experiment_data_amount_LV_fourier(grid_res))
    #        exp_dict_list_flatten = [item for sublist in exp_dict_list for item in sublist]
    #        df = pd.DataFrame(exp_dict_list_flatten)
    #        df.to_csv('examples\\AAAI_expetiments\\results\\LV_lam_{}.csv'.format(grid_res))
    ...
    #        exp_dict_list_flatten = [item for sublist in exp_dict_list for item in sublist]
    #        df = pd.DataFrame(exp_dict_list_flatten)
    #        df.to_csv('examples\\AAAI_expetiments\\results\\LV_NGD_{}.csv'.format(grid_res))
