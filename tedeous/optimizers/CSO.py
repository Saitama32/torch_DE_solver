...
class PSO(torch.optim.Optimizer):
    ...
    def step(self, closure=None) -> torch.Tensor:
        ...
        next_swarm = torch.zeros_like(self.swarm)
        grad_mask = torch.zeros(self.pop_size, dtype=torch.bool)
        ...
        while U:
            ...
            # Шаг 13: добавление победителя в P(t + 1)
            next_swarm[iw] = Xw
            grad_mask[il] = True
            
            # Шаг 14: обновление проигравшего
            ...
            # print(il)
            # print(Xl_new)
            next_swarm[il] = Xl_new

            # Шаг 16: удаление X1(t), X2(t) из U
            U.remove(i)
            U.remove(j)
        ...
        self.loss_swarm, self.grads_swarm = closure()
        # if self.use_grad: 
        #     self.swarm = next_swarm - self.gradient_descent()
        # else:
        #     self.swarm = next_swarm
        
        # self.swarm = torch.where((next_swarm < -1.0) | (next_swarm > 1.0), self.swarm, next_swarm)
