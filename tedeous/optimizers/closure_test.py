...
class Closure():
    ...
    def _closure_pso(self):
        ...
        for particle in self.optimizer.swarm:
            ...
            loss_swarm_num.append(loss_particle.item())
            grads_swarm.append(grads.reshape(1, -1))

        # for particle in self.optimizer.f_p:
        #     loss_swarm_num.append()

        losses = torch.stack(loss_swarm).reshape(-1)
        gradients = torch.vstack(grads_swarm)
        # self.learning_swarm_losses["learning_swarm_losses"].append(self.optimizer.f_p)
        self.model.dict_of_learning["learning_swarm_losses"].append(loss_swarm_num)
